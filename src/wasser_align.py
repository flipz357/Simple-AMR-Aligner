...
def build_arg_parser():
    ...
    parser.add_argument('-a'
            , type=str
            , help='file path to first SemBank (contains AMR + sentences)')
...
if __name__ == "__main__":
    ...
    logger.info("loading amrs and sents from file {}".format(
        args.a))
    ...
    grapa_sent = gh.GraphParser(input_format="sentence", 
                            edge_to_node_transform=args.edge_to_node_transform)
    
    string_graphs, string_sents = dh.read_amr_file(graphfile1)
    graphs1, nodemap1 = grapa.parse(string_graphs)
    graphs2, nodemap2 = grapa_sent.parse(string_sents)
