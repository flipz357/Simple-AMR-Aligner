...
class GraphParser():
    ...
    def __init__(self, input_format="penman", edge_to_node_transform=False):
        ...
        if input_format not in ["penman", "tsv", "sentence"]:
    ...
    def graphs_to_triples(self, string_graphs):
        ...
        if self.input_format == "sentence":
            nlp = spacy.load("en_core_web_sm")
            tripless = [sent2triples(s, nlp) for s in string_graphs]
...
def sent2triples(sent, nlp):
    ...
    for i, token in enumerate(doc):
        ...
        ch = token.lemma_
        triple = (str(token.head.idx)+ "_" + head, ":" + rel, str(token.idx) + "_" + ch)
        triples.append(triple)
        triples.append((str(token.idx) + "_" + ch, ":instance", ch))
    return triples
